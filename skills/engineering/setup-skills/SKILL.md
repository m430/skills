---
name: setup-skills
description: "为本仓库配置工程技能：迭代工作区约定、领域文档布局、shadcn 项目的设计系统登记。在首次使用其他工程技能之前运行一次。"
...
- **设计系统**：shadcn 项目在仓库根目录留一份 `design.md`（记下在底座上改了哪些令牌与组件），让写 UI 的会话先读它
...
- shadcn 信号：`components.json`（shadcn 的配置文件，在仓库根目录或某个包的目录下）。C 节只在这个信号存在时成立：单独一个 `src/components/` 目录不算，接上底座的仓库必然有 `components.json`。

### 2. 呈现发现并提问

总结已存在和缺失的内容。然后按顺序处理各节：一节、一个回答，再进入下一节。

每节以推荐答案开头，让用户一个词就能接受。运行环境提供结构化提问工具（`AskUserQuestion`）时用它提问，一次一个问题、选项即候选答案、推荐项放第一位；不可用时退回文本。只有当选项真正分叉时才给一行说明；探索已经定案时整节跳过（无 monorepo 时跳过 B 节的多上下文选项，不是 shadcn 项目时跳过 C 节）。
...
**C 节：设计系统。** 探索到 shadcn 信号的仓库才有这一节，没有就整节跳过：`design.md` 由 `design-from-image` 在 shadcn 底座上产出，非 shadcn 项目跑不了。位置固定在仓库根目录，没有配置项；要定的是什么时候把它登记进约定。

- **`design.md` 已存在**：无需提问，把它写进第 4 步的块。
- **是 shadcn 项目但还没有 `design.md`**：问一个问题：现在就登记，还是等产物出来再登记。推荐现在登记，块里写的是「写 UI 之前先跑 `/design-from-image`」，而不是指向一份还不存在的文件。
...
[只有 C 节成立时才有这一段，不是 shadcn 项目就整段删掉。`design.md` 已存在时写「写任何 UI 之前先读 `design.md`：本项目在 shadcn 底座上改了哪些令牌与组件都在里面。颜色、间距、圆角一律走令牌类名（`bg-primary`、`text-muted-foreground` 这一类），不要在组件里写死色值。」尚未产出时写「写任何 UI 之前先跑 `/design-from-image` 产出 `design.md`，之后一律先读它。」`design-from-image` 写出 `design.md` 后会把这一段换成读文件的说法。]
```
...
shadcn 项目如果还没有 `design.md`，提一句：带一张设计参考图跑 `/design-from-image` 就能产出，它会自己把块里那一节改成读文件的说法。
